    ...
    NavigationStack {
      GeometryReader { geometry in
        VStack {
          ...
          HStack(spacing: 20) { // This spacing is the padding between the two columns
            Text("Discover")
                .font(.headline)
                .padding(.top, 8)
                ...
                .frame(maxWidth: .infinity, alignment: .leading)
            ScrollView {
              DiscoverSection(historyItems: historyItems, mangas: mangas, loadMoreMangas: loadMoreMangas)
            }
                .frame(width: (geometry.size.width / 2) - 10) // Half the width minus half the spacing
          }
